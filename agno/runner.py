...
try:
    ...
    print(
        "Please install them using: uv pip install agno anthropic google-generativeai"
    )
    sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Agno Skill Runner - Test security skills using LocalSkills"
    )
    parser.add_argument(
        "skill_dir", help="Path to the skill directory (e.g., skills/suricata-analyst)"
    )
    parser.add_argument("prompt", help="The prompt/task to execute")
    parser.add_argument(
        "--model",
        default="gemini-3-flash-preview",
        help="Model to use (e.g., gemini-1.5-flash, claude-3-5-sonnet-20240620)",
    )
